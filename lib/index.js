...
const prepareBody = (data, code = 500) => {
  ...
  if (!data) {
    const status = code;
    const message = common.STATUS_CODES[code];
    const description = common.STATUS_CODES[code];
    return { status, code, message, description };
  }

  // handle error or data
  const stack = !env.isProduction();
  let body = {};
  body = _.isError(data) ? common.mapErrorToObject(data, { code, stack }) : data;
...
const respond = (request, response, next) => {
  ...
  _.forEach(common.STATUS_CODES, mapStatusToMethod(response));
